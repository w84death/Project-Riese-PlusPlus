...
ig.GameFlow = ig.Class.extend({
	...
	init: function(){
		...
		ig.input.bind( ig.KEY.X, 'change' );
		ig.input.bind( ig.KEY.SPACE, 'jump' );
		ig.input.bind( ig.KEY.ENTER, 'a' );
		...
		ig.input.bind( ig.GAMEPAD.FACE_3, 'x');
		ig.input.bind( ig.GAMEPAD.FACE_4, 'change');
	...
	startNewGame: function (params) {
        if(ig.game.setup.map == 'sandbox'){
			ig.game.director.jumpTo(LevelSandbox);
        }
        if(ig.game.setup.map == 'e1m1'){
            ig.game.director.jumpTo(LevelE1m1);
        }
        if(ig.game.setup.map == 'e1m2'){
            ig.game.director.jumpTo(LevelE1m2);
        }

        this.selectHero(params.hero);

        //ig.game.camera.reset();
	...
	nextLevel: function(params){
        ...
        var spawnX = (ig.system.width*0.5) + ig.game.screen.x;
        if(inHand){
 			ig.game.spawnEntity( EntityRifle,spawnX,20,{ammo:ammo});
 		}

        this.spawnPlayer({
        	hero: params.hero || hero,
        	x: spawnX,
        	y: 0
        });
	},
    ...
    selectHero: function(hero) {
        if(hero === 0){
            ig.GameState.selectedHero = ig.EntityStrongMan;
        }
        if(hero === 1){
            ig.GameState.selectedHero = ig.EntityMcCthulhu;
        }
    },
